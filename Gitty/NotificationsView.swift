...
struct NotificationsView: View {
	...
	/// Only filters done notifications that are still read - if a done notification
	/// becomes unread again (new activity), it should be shown
	private var visibleNotifications: [GitHubNotification] {
		...
		return
			...
			.filter { notification in
				// Filter out done notifications, but only if they're still read
				// If a done notification becomes unread (new activity), show it
				if doneIds.contains(notification.id) && !notification.unread {
					return false
				}
